...
public class Rec_Cherry_Pickup_2 {
    ...
    public static int func(int i,int j1,int j2,int m,int n,int[][] grid){
        ...
        for(int robo1 = -1 ; robo1 <= 1;robo1++){ //for every robo1 moves
            for(int robo2 = -1 ; robo2 <= 1;robo2++){  // robo2 moves 3 times (3*3) = 9
                ...
                if(j1 == j2) {
                    maxi = Math.max(maxi,grid[i][j1] + func(i+1,j1+robo1,j2+robo2,m,n,grid));
                }else{
                    maxi = Math.max(maxi,grid[i][j1] + grid[i][j2] + func(i+1,j1+robo1,j2+robo2,m,n,grid));
                }
            }
        }
        return maxi;
